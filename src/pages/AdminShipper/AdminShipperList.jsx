...
import { callAPIgetAccountShipper, callAPIgetListShipper, callAPIgetListStation } from "../../redux/action/acction";
...
function applySortFilter(array, comparator, query) {
    ...
    if (query) {
        return filter(
            array,
            (_stations) =>
                _stations.account.profile.fullName.toLowerCase().indexOf(query.toLowerCase()) !== -1
        );
    }
...
export default function AdminShipperList() {
    ...
    const handleDelete = (id, name) => {
        API("PUT", URL_API + `/shippers/status/${id}`, null, token).then(
            (res) => {
                try {
