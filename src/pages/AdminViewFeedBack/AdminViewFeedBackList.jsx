...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_kitchen) =>
        _kitchen.kitchenName.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function AdminViewFeedBackList() {
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - ADMINVIEWFEEDBACKLIST.length)
      : 0;
              ...
              <Table>
                ...
                <TableBody>
                  {filteredKitchen
                    ...
                    .map((row) => {
                      const { id, name, phone, kitchen, order, star, status } =
                        row;
